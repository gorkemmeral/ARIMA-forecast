...
plot <-
  ggplot(sales, aes(visit_date, orders, group = 1)) + geom_line() + ylab("Sales") + xlab("Date")
sales_ts = ts(sales[, c('orders')])
sales$clean_orders = tsclean(sales_ts)
clean_plot <-
  ggplot() + geom_line(data = sales, aes(x = visit_date, y = clean_orders, group =
                                           1)) + ylab('Cleaned Orders')
